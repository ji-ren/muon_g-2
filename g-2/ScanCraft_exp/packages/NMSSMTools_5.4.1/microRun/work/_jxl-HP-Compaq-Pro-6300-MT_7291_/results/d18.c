...
extern DNN S18_omg_p25p25_u;
REAL S18_omg_p25p25_u(double GG, REAL * momenta,REAL*cb_coeff,int * err)
{REAL  ans=0;
REAL DP[6];
...
width[10]=0.; mass[10]=V[23];  Qtxt[10]="\1\3";
width[9]=0.; mass[9]=V[23];  Qtxt[9]="\1\4";
width[8]=0.; mass[8]=V[22];  Qtxt[8]="\1\3";
width[7]=0.; mass[7]=V[22];  Qtxt[7]="\1\4";
...
N=-C[80];
D=+C[81];
R=+DP[1]*(C[86]*(DP[1]-DP[0])-C[87])+C[89]+C[88]*DP[0];
R*=(N/D);
...
N=-C[80];
D=+C[81];
R=+DP[1]*(C[95]*(DP[1]-DP[0])+C[96])+C[98]+C[97]*DP[0];
R*=(N/D);
...
N=-C[80];
D=+C[81];
R=+DP[1]*(C[95]*(DP[1]-DP[0])-C[101])+C[103]+C[102]*DP[0];
R*=(N/D);
...
N=-C[80];
D=+C[81];
R=+DP[1]*(C[110]*(DP[1]-DP[0])-C[111])+C[113]+C[112]*DP[0];
R*=(N/D);
...
C[114]=+V[5]*(V[15]*(V[230]*(V[231]*(4*(S[0]+S[1])))));
S[4]=V[231]*V[231]*V[231];
S[5]=V[231]*V[231]*V[231]*V[231];
C[113]=+S[2]*(V[230]*(V[230]*(V[5]*(V[5]*(2*S[0]-S[1])+2*V[231]*V[230]*
 V[15])+S[2]*(2*S[0]+S[1]))+2*S[4]*V[15]*V[5])+S[5]*(S[2]-S[3]));
C[112]=+V[15]*(V[230]*(V[230]*(V[15]*(2*S[0]+S[1])+2*V[231]*V[230]*V[5])+2*
 S[4]*V[5])+S[5]*V[15])+4*S[0]*S[1]*S[3];
C[111]=+4*S[0]*S[1]*S[2];
C[110]=+4*S[0]*S[1];
C[109]=+S[2]*(V[15]*(V[230]*(V[230]*(V[15]*(2*S[0]+S[1])+4*V[231]*V[230]*
 V[5])+4*S[4]*V[5])+S[5]*V[15])+4*S[0]*S[1]*S[3]);
C[108]=+V[15]*(V[230]*(V[230]*(V[15]*(4*S[0]+2*S[1])+4*V[231]*V[230]*V[5])+
 4*S[4]*V[5])+2*S[5]*V[15]);
C[107]=+S[1]*(4*S[0]+2*S[1])+2*S[5];
C[106]=+S[1]*(2*S[0]+S[1])+S[5];
S[6]=V[229]*V[229];
C[105]=+S[3]*(S[2]*(V[228]*(V[231]*(V[231]*V[228]+2*V[230]*V[229]))+S[1]*
 S[6]));
C[104]=+V[5]*(V[15]*(2*(V[228]*(V[229]*(S[0]+S[1])+V[231]*V[230]*V[228])+
 V[231]*V[230]*S[6])));
C[103]=+S[2]*(V[228]*(V[230]*(V[5]*(V[5]*(2*V[231]*V[229]-V[230]*V[228])+2*
 V[230]*V[229]*V[15])+S[2]*(2*V[231]*V[229]+V[230]*V[228]))+2*S[0]*V[229]*
 V[15]*V[5])+S[6]*(S[0]*(S[2]-S[3])));
C[102]=+V[228]*(V[5]*(V[229]*(2*(V[15]*(S[0]+S[1])+V[231]*V[230]*V[5]))+
 S[0]*V[228]*V[5])+S[2]*(V[230]*(2*V[231]*V[229]+V[230]*V[228])))+S[6]*(
 S[0]*S[2]+S[1]*S[3]);
C[101]=+V[15]*(V[5]*(2*(V[228]*(V[229]*(S[0]+S[1])-V[231]*V[230]*V[228])-
 V[231]*V[230]*S[6]))+4*V[231]*V[230]*V[229]*V[228]*V[15]);
S[7]=V[228]*V[228];
C[100]=+4*S[6]*S[7]*S[2]*S[3];
C[99]=+V[5]*(V[15]*(V[228]*(V[229]*(4*(S[6]+S[7])))));
C[98]=+S[2]*(V[230]*(V[228]*(V[5]*(V[5]*(2*V[231]*V[229]-V[230]*V[228])+2*
 V[231]*V[228]*V[15])+S[2]*(2*V[231]*V[229]+V[230]*V[228]))+2*V[231]*S[6]*
 V[15]*V[5])+S[6]*(S[0]*(S[2]-S[3])));
C[97]=+V[230]*(V[5]*(V[229]*(V[5]*(2*V[231]*V[228]+V[230]*V[229])+2*V[231]*
 V[229]*V[15])+2*V[231]*S[7]*V[15])+S[2]*(V[228]*(2*V[231]*V[229]+V[230]*
 V[228])))+S[0]*(S[6]*S[2]+S[7]*S[3]);
C[96]=+V[15]*(V[5]*(2*(V[228]*(V[229]*(S[0]+S[1])-V[231]*V[230]*V[228])-
 V[231]*V[230]*S[6]))-4*V[231]*V[230]*V[229]*V[228]*V[15]);
C[95]=+4*V[231]*V[230]*V[229]*V[228];
C[94]=+S[2]*(V[5]*(V[228]*(V[231]*(2*(V[15]*(V[231]*V[229]+V[230]*V[228])+
 V[230]*V[229]*V[5])+V[231]*V[228]*V[5])+2*S[1]*V[229]*V[15])+S[6]*(V[230]*(
 2*V[231]*V[15]+V[230]*V[5])))+S[2]*(V[228]*(V[230]*(2*V[231]*V[229]+V[230]*
 V[228]))+S[0]*S[6]));
C[93]=+V[15]*(2*(V[5]*(V[228]*(V[229]*(S[0]+S[1])+V[231]*V[230]*V[228])+
 V[231]*V[230]*S[6])+V[15]*(S[0]*S[6]+S[1]*S[7]))+4*V[231]*V[230]*V[229]*
 V[228]*V[15]);
C[92]=+V[228]*(V[230]*(4*V[231]*V[229]+2*V[230]*V[228]))+2*S[0]*S[6];
C[91]=+V[228]*(V[230]*(2*V[231]*V[229]+V[230]*V[228]))+S[0]*S[6];
...
S[12]=V[229]*V[229]*V[229];
S[13]=V[229]*V[229]*V[229]*V[229];
C[89]=+S[2]*(V[228]*(V[228]*(V[5]*(V[5]*(2*S[6]-S[7])+2*V[229]*V[228]*
 V[15])+S[2]*(2*S[6]+S[7]))+2*S[12]*V[15]*V[5])+S[13]*(S[2]-S[3]));
C[88]=+V[15]*(V[228]*(V[228]*(V[15]*(2*S[6]+S[7])+2*V[229]*V[228]*V[5])+2*
 S[12]*V[5])+S[13]*V[15])+4*S[6]*S[7]*S[3];
C[87]=+4*S[6]*S[7]*S[2];
C[86]=+4*S[6]*S[7];
C[85]=+S[2]*(V[15]*(V[228]*(V[228]*(V[15]*(2*S[6]+S[7])+4*V[229]*V[228]*
 V[5])+4*S[12]*V[5])+S[13]*V[15])+4*S[6]*S[7]*S[3]);
C[84]=+V[15]*(V[228]*(V[228]*(V[15]*(4*S[6]+2*S[7])+4*V[229]*V[228]*V[5])+4*
...
C[80]=+S[14];
S[15]=V[1]*V[1]*V[1];
C[79]=+V[468]*V[126]*V[5]*S[15];
S[16]=V[468]*V[468];
S[17]=V[126]*V[126];
S[18]=V[1]*V[1];
C[78]=+3*S[16]*S[17]*S[3]*S[18];
S[19]=V[15]*V[15]*V[15];
C[77]=+S[19]*(V[5]*(S[0]+S[1]));
C[76]=+V[5]*(V[15]*(S[0]+S[1]));
C[75]=+S[19]*(V[5]*(S[0]+S[1])+2*V[231]*V[230]*V[15]);
C[74]=+V[15]*(V[5]*(S[0]+S[1])+4*V[231]*V[230]*V[15]);
C[73]=+2*V[231]*V[230]*S[2];
C[72]=+S[19]*(V[5]*(S[6]+S[7]));
C[71]=+V[5]*(V[15]*(S[6]+S[7]));
C[70]=+S[19]*(V[5]*(S[6]+S[7])+2*V[229]*V[228]*V[15]);
C[69]=+V[15]*(V[5]*(S[6]+S[7])+4*V[229]*V[228]*V[15]);
C[68]=+2*V[229]*V[228]*S[2];
S[20]=V[121]*V[121]*V[121];
S[21]=V[118]*V[118];
C[67]=+12*V[122]*S[20]*S[9]*S[21]*V[7]*V[6]*S[11];
C[66]=+V[466]*V[125]*V[5]*S[15];
S[22]=V[122]*V[122];
S[23]=V[121]*V[121];
S[24]=V[7]*V[7];
S[25]=V[6]*V[6];
C[65]=+2*S[22]*S[23]*S[9]*S[24]*S[25]*S[11];
C[64]=+3*V[468]*V[466]*V[126]*V[125]*S[3]*S[18];
...
S[27]=V[466]*V[466];
S[28]=V[125]*V[125];
C[60]=+3*S[27]*S[28]*S[3]*S[18];
C[59]=+V[464]*V[95]*V[5]*S[15];
S[29]=V[464]*V[464];
S[30]=V[95]*V[95];
C[58]=+3*S[29]*S[30]*S[3]*S[18];
C[57]=+V[462]*V[92]*V[5]*S[15];
C[56]=+3*V[464]*V[462]*V[95]*V[92]*S[3]*S[18];
S[31]=V[462]*V[462];
S[32]=V[92]*V[92];
C[55]=+3*S[31]*S[32]*S[3]*S[18];
C[54]=+S[2]*(V[5]*(V[15]*(S[0]+S[1])-2*V[231]*V[230]*V[5])+2*V[231]*V[230]*
 S[2]);
C[53]=+V[5]*(V[15]*(S[0]+S[1])+2*V[231]*V[230]*V[5])+2*V[231]*V[230]*S[2];
C[52]=+S[2]*(V[5]*(V[15]*(S[0]+S[1])+2*V[231]*V[230]*V[5]));
C[51]=+V[5]*(V[15]*(S[0]+S[1])-2*V[231]*V[230]*V[5])+2*V[231]*V[230]*S[2];
C[50]=+V[15]*(2*(V[5]*(S[0]+S[1])+V[231]*V[230]*V[15]));
C[49]=+2*V[231]*V[230];
C[48]=+S[2]*(V[5]*(V[15]*(S[6]+S[7])-2*V[229]*V[228]*V[5])+2*V[229]*V[228]*
 S[2]);
C[47]=+V[5]*(V[15]*(S[6]+S[7])+2*V[229]*V[228]*V[5])+2*V[229]*V[228]*S[2];
C[46]=+S[2]*(V[5]*(V[15]*(S[6]+S[7])+2*V[229]*V[228]*V[5]));
C[45]=+V[5]*(V[15]*(S[6]+S[7])-2*V[229]*V[228]*V[5])+2*V[229]*V[228]*S[2];
C[44]=+V[15]*(2*(V[5]*(S[6]+S[7])+V[229]*V[228]*V[15]));
C[43]=+2*V[229]*V[228];
C[42]=+12*V[122]*S[8]*S[9]*S[21]*V[7]*S[11];
C[41]=+V[460]*V[89]*V[5]*S[15];
C[40]=+3*V[464]*V[460]*V[95]*V[89]*S[3]*S[18];
C[39]=+2*S[22]*S[8]*S[9]*S[24]*S[11];
C[38]=+3*V[462]*V[460]*V[92]*V[89]*S[3]*S[18];
C[37]=+S[2]*(S[2]-2*S[3]);
C[36]=+2*S[3];
C[35]=+4*S[22]*S[8]*S[9]*S[24]*S[11];
S[33]=V[460]*V[460];
S[34]=V[89]*V[89];
C[34]=+3*S[33]*S[34]*S[3]*S[18];
S[35]=V[119]*V[119];
S[36]=V[2]*V[2];
C[33]=+S[2]*(S[36]*(V[5]*(V[5]*(S[35]*(4*(S[0]-S[1]))-6*S[0]-3*S[1])+6*
 V[231]*V[230]*V[15])+S[2]*(S[35]*(4*(S[0]-S[1]))+3*S[1]))+S[3]*(S[2]*(6*(
 S[0]+S[1]))));
C[32]=+S[36]*(S[35]*(4*(S[3]*(S[1]-S[0])+S[2]*(S[0]-S[1])))+V[5]*(V[231]*(3*
 V[231]*V[5]+6*V[230]*V[15]))+3*S[1]*S[2])+S[3]*(S[2]*(6*(S[0]+S[1])));
C[31]=+V[5]*(V[15]*(V[230]*(V[231]*(12*S[2]-18*S[36]))));
C[30]=+12*V[231]*V[230]*V[15]*V[5];
C[29]=+S[2]*(S[36]*(V[5]*(V[5]*(S[35]*(4*(S[1]-S[0]))+6*S[0]+3*S[1])+12*
 V[231]*V[230]*V[15])+S[2]*(S[35]*(4*(S[0]-S[1]))+3*S[1]))+S[2]*(V[5]*(V[5]*
 (6*(-S[0]-S[1]))-12*V[231]*V[230]*V[15])));
C[28]=+S[36]*(S[35]*(S[3]*(4*(S[0]-S[1]))+S[2]*(12*(S[0]-S[1])))+V[5]*(
 V[231]*(12*V[230]*V[15]-3*V[231]*V[5]))+9*S[1]*S[2])+S[2]*(V[5]*(V[5]*(6*(-
 S[0]-S[1]))-24*V[231]*V[230]*V[15]));
C[27]=+S[36]*(S[35]*(8*(S[0]-S[1]))+6*S[1])-12*V[231]*V[230]*V[15]*V[5];
C[26]=+V[15]*(S[36]*(V[15]*(S[35]*(16*(S[0]-S[1]))+12*S[1])+18*V[231]*
 V[230]*V[5])-12*V[231]*V[230]*S[2]*V[5]);
C[25]=+S[36]*(S[35]*(16*(S[0]-S[1]))+12*S[1])-12*V[231]*V[230]*V[15]*V[5];
C[24]=+S[36]*(S[35]*(8*(S[0]-S[1]))+6*S[1]);
C[23]=+S[2]*(S[36]*(V[5]*(V[5]*(S[35]*(4*(S[6]-S[7]))-6*S[6]-3*S[7])+6*
 V[229]*V[228]*V[15])+S[2]*(S[35]*(4*(S[6]-S[7]))+3*S[7]))+S[3]*(S[2]*(6*(
 S[6]+S[7]))));
C[22]=+S[36]*(S[35]*(4*(S[3]*(S[7]-S[6])+S[2]*(S[6]-S[7])))+V[5]*(V[229]*(3*
 V[229]*V[5]+6*V[228]*V[15]))+3*S[7]*S[2])+S[3]*(S[2]*(6*(S[6]+S[7])));
C[21]=+V[5]*(V[15]*(V[228]*(V[229]*(12*S[2]-18*S[36]))));
C[20]=+12*V[229]*V[228]*V[15]*V[5];
C[19]=+S[2]*(S[36]*(V[5]*(V[5]*(S[35]*(4*(S[7]-S[6]))+6*S[6]+3*S[7])+12*
 V[229]*V[228]*V[15])+S[2]*(S[35]*(4*(S[6]-S[7]))+3*S[7]))+S[2]*(V[5]*(V[5]*
 (6*(-S[6]-S[7]))-12*V[229]*V[228]*V[15])));
C[18]=+S[36]*(S[35]*(S[3]*(4*(S[6]-S[7]))+S[2]*(12*(S[6]-S[7])))+V[5]*(
 V[229]*(12*V[228]*V[15]-3*V[229]*V[5]))+9*S[7]*S[2])+S[2]*(V[5]*(V[5]*(6*(-
 S[6]-S[7]))-24*V[229]*V[228]*V[15]));
C[17]=+S[36]*(S[35]*(8*(S[6]-S[7]))+6*S[7])-12*V[229]*V[228]*V[15]*V[5];
C[16]=+V[15]*(S[36]*(V[15]*(S[35]*(16*(S[6]-S[7]))+12*S[7])+18*V[229]*
 V[228]*V[5])-12*V[229]*V[228]*S[2]*V[5]);
C[15]=+S[36]*(S[35]*(16*(S[6]-S[7]))+12*S[7])-12*V[229]*V[228]*V[15]*V[5];
C[14]=+S[36]*(S[35]*(8*(S[6]-S[7]))+6*S[7]);
S[37]=V[3]*V[3];
C[13]=+36*S[23]*S[9]*S[10]*S[37]*S[36];
C[12]=+V[458]*S[14];
C[11]=+3*V[468]*V[458]*V[126]*V[15]*S[3]*S[15];
C[10]=+S[2]*(2*S[2]-S[36]);
C[9]=+4*S[2]-S[36];
C[8]=+2*V[122]*V[121]*S[9]*S[21]*V[7]*V[6]*S[37]*S[36];
C[7]=+3*V[466]*V[458]*V[125]*V[15]*S[3]*S[15];
C[6]=+S[3]*(S[2]*(S[36]*(S[36]*(S[35]*(32*S[35]-24)-9)+36*S[2])-36*S[26]));
C[5]=+S[36]*(S[36]*(S[35]*(S[3]*(32*S[35]-24)+S[2]*(32*S[35]-24))+9*S[2])-
 36*S[2]*S[3])+72*S[26]*S[3];
S[38]=V[2]*V[2]*V[2]*V[2];
C[4]=+S[38]*(S[35]*(32*S[35]-24)+9)+36*S[2]*S[3];
C[3]=+S[38]*(S[2]*(S[35]*(64*S[35]-48)+18));
