/*******************************
*    CalcHEP  3.6.28*
*******************************/
#include"num_in.h"
#include"num_out.h"
static void C37(REAL *);
extern DNN S37_omg_p25p25_u;
REAL S37_omg_p25p25_u(double GG, REAL * momenta,REAL*cb_coeff,int * err)
{REAL  ans=0;
REAL DP[6];
REAL* V=va_omg_p25p25_u;
REAL mass[14],width[14];
char * Qtxt[14];
REAL Q0[14]; COMPLEX Q1[14]; REAL Q2[14];
 if(momenta)
 {width[3]=V[554]; mass[3]=V[9];  Qtxt[3]="\1\2";
width[2]=V[555]; mass[2]=V[10];  Qtxt[2]="\1\2";
width[1]=V[556]; mass[1]=V[11];  Qtxt[1]="\1\2";
width[13]=0.; mass[13]=V[15];  Qtxt[13]="\1\4";
width[12]=0.; mass[12]=V[15];  Qtxt[12]="\1\3";
width[11]=0.; mass[11]=V[16];  Qtxt[11]="\1\4";
width[10]=0.; mass[10]=V[16];  Qtxt[10]="\1\3";
width[9]=0.; mass[9]=V[17];  Qtxt[9]="\1\4";
width[8]=0.; mass[8]=V[17];  Qtxt[8]="\1\3";
width[7]=0.; mass[7]=V[18];  Qtxt[7]="\1\4";
width[6]=0.; mass[6]=V[18];  Qtxt[6]="\1\3";
width[5]=0.; mass[5]=V[19];  Qtxt[5]="\1\4";
width[4]=0.; mass[4]=V[19];  Qtxt[4]="\1\3";
*err=*err|prepDen(13,nin_omg_p25p25_u,BWrange_omg_p25p25_u*
 BWrange_omg_p25p25_u,mass,width,Qtxt,momenta,Q0,Q1,Q2);
sprod_(4, momenta, DP);
}
{
REAL N,D,R; COMPLEX Prop;
static REAL C[122];                                                   
if(!momenta){ C37(C); return 0;}
  REAL N_p1p2_=1/DP[0];
N=-C[0];
D=+C[1];
R=+DP[1]*(2*(DP[1]-DP[0])+C[2])+C[4]-C[3]*DP[0];
R*=(N/D);
Prop=1*Q2[4];
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[0];
D=+C[1];
R=+DP[1]*(2*(DP[1]-DP[0])-C[5])+C[6]*DP[0]-C[7];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[4]):Q1[4])*(gtwidth_omg_p25p25_u? creal(Q1[5]):conj(
 Q1[5]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[8];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[10])+C[12]-C[11]*DP[0];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[4]):Q1[4])*(gtwidth_omg_p25p25_u? creal(Q1[6]):conj(
 Q1[6]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[8];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[13])+C[14]*DP[0]-C[15];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[4]):Q1[4])*(gtwidth_omg_p25p25_u? creal(Q1[7]):conj(
 Q1[7]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[16];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[17])+C[19]-C[18]*DP[0];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[4]):Q1[4])*(gtwidth_omg_p25p25_u? creal(Q1[8]):conj(
 Q1[8]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[16];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[20])+C[21]*DP[0]-C[22];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[4]):Q1[4])*(gtwidth_omg_p25p25_u? creal(Q1[9]):conj(
 Q1[9]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[23];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[24])+C[26]-C[25]*DP[0];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[4]):Q1[4])*(gtwidth_omg_p25p25_u? creal(Q1[10]):conj(
 Q1[10]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[23];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[27])+C[28]*DP[0]-C[29];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[4]):Q1[4])*(gtwidth_omg_p25p25_u? creal(Q1[11]):conj(
 Q1[11]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[30];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[31])-C[33]-C[32]*DP[0];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[4]):Q1[4])*(gtwidth_omg_p25p25_u? creal(Q1[12]):conj(
 Q1[12]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[30];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[31])-C[33]-C[32]*DP[0];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[4]):Q1[4])*(gtwidth_omg_p25p25_u? creal(Q1[13]):conj(
 Q1[13]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[34];
D=+C[35];
R=+C[38]-C[37]*DP[0]+C[36]*DP[1];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[4]):Q1[4])*(gswidth_omg_p25p25_u? creal(Q1[1]):conj(
 Q1[1]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[39];
D=+C[35];
R=+C[38]-C[37]*DP[0]+C[36]*DP[1];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[4]):Q1[4])*(gswidth_omg_p25p25_u? creal(Q1[2]):conj(
 Q1[2]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[40];
D=+C[35];
R=+C[38]-C[37]*DP[0]+C[36]*DP[1];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[4]):Q1[4])*(gswidth_omg_p25p25_u? creal(Q1[3]):conj(
 Q1[3]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[41];
D=+C[1];
R=+DP[1]*(2*(DP[1]-DP[0])+C[42])+C[44]-C[43]*DP[0];
R*=(N/D);
Prop=1*Q2[6];
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[41];
D=+C[1];
R=+DP[1]*(2*(DP[1]-DP[0])-C[5])+C[45]*DP[0]-C[46];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[6]):Q1[6])*(gtwidth_omg_p25p25_u? creal(Q1[7]):conj(
 Q1[7]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[47];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[48])+C[50]-C[49]*DP[0];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[6]):Q1[6])*(gtwidth_omg_p25p25_u? creal(Q1[8]):conj(
 Q1[8]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[47];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[51])+C[52]*DP[0]-C[53];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[6]):Q1[6])*(gtwidth_omg_p25p25_u? creal(Q1[9]):conj(
 Q1[9]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[54];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[55])+C[57]-C[56]*DP[0];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[6]):Q1[6])*(gtwidth_omg_p25p25_u? creal(Q1[10]):conj(
 Q1[10]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[54];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[58])+C[59]*DP[0]-C[60];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[6]):Q1[6])*(gtwidth_omg_p25p25_u? creal(Q1[11]):conj(
 Q1[11]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[61];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[62])-C[64]-C[63]*DP[0];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[6]):Q1[6])*(gtwidth_omg_p25p25_u? creal(Q1[12]):conj(
 Q1[12]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[61];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[62])-C[64]-C[63]*DP[0];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[6]):Q1[6])*(gtwidth_omg_p25p25_u? creal(Q1[13]):conj(
 Q1[13]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[65];
D=+C[35];
R=+C[67]-C[66]*DP[0]+C[36]*DP[1];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[6]):Q1[6])*(gswidth_omg_p25p25_u? creal(Q1[1]):conj(
 Q1[1]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[68];
D=+C[35];
R=+C[67]-C[66]*DP[0]+C[36]*DP[1];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[6]):Q1[6])*(gswidth_omg_p25p25_u? creal(Q1[2]):conj(
 Q1[2]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[69];
D=+C[35];
R=+C[67]-C[66]*DP[0]+C[36]*DP[1];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[6]):Q1[6])*(gswidth_omg_p25p25_u? creal(Q1[3]):conj(
 Q1[3]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[70];
D=+C[1];
R=+DP[1]*(2*(DP[1]-DP[0])+C[71])+C[73]-C[72]*DP[0];
R*=(N/D);
Prop=1*Q2[8];
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[70];
D=+C[1];
R=+DP[1]*(2*(DP[1]-DP[0])-C[5])+C[74]*DP[0]-C[75];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[8]):Q1[8])*(gtwidth_omg_p25p25_u? creal(Q1[9]):conj(
 Q1[9]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[76];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[77])+C[79]-C[78]*DP[0];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[8]):Q1[8])*(gtwidth_omg_p25p25_u? creal(Q1[10]):conj(
 Q1[10]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[76];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[80])+C[81]*DP[0]-C[82];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[8]):Q1[8])*(gtwidth_omg_p25p25_u? creal(Q1[11]):conj(
 Q1[11]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[83];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[84])-C[86]-C[85]*DP[0];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[8]):Q1[8])*(gtwidth_omg_p25p25_u? creal(Q1[12]):conj(
 Q1[12]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[83];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[84])-C[86]-C[85]*DP[0];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[8]):Q1[8])*(gtwidth_omg_p25p25_u? creal(Q1[13]):conj(
 Q1[13]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[87];
D=+C[35];
R=+C[89]-C[88]*DP[0]+C[36]*DP[1];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[8]):Q1[8])*(gswidth_omg_p25p25_u? creal(Q1[1]):conj(
 Q1[1]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[90];
D=+C[35];
R=+C[89]-C[88]*DP[0]+C[36]*DP[1];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[8]):Q1[8])*(gswidth_omg_p25p25_u? creal(Q1[2]):conj(
 Q1[2]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[91];
D=+C[35];
R=+C[89]-C[88]*DP[0]+C[36]*DP[1];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[8]):Q1[8])*(gswidth_omg_p25p25_u? creal(Q1[3]):conj(
 Q1[3]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[92];
D=+C[1];
R=+DP[1]*(2*(DP[1]-DP[0])+C[93])+C[95]-C[94]*DP[0];
R*=(N/D);
Prop=1*Q2[10];
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[92];
D=+C[1];
R=+DP[1]*(2*(DP[1]-DP[0])-C[5])+C[96]*DP[0]-C[97];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[10]):Q1[10])*(gtwidth_omg_p25p25_u? creal(Q1[11]):conj(
 Q1[11]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[98];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[99])-C[101]-C[100]*DP[0];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[10]):Q1[10])*(gtwidth_omg_p25p25_u? creal(Q1[12]):conj(
 Q1[12]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[98];
D=+C[9];
R=+DP[1]*(2*(DP[1]-DP[0])+C[99])-C[101]-C[100]*DP[0];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[10]):Q1[10])*(gtwidth_omg_p25p25_u? creal(Q1[13]):conj(
 Q1[13]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[102];
D=+C[35];
R=+C[104]-C[103]*DP[0]+C[36]*DP[1];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[10]):Q1[10])*(gswidth_omg_p25p25_u? creal(Q1[1]):conj(
 Q1[1]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[105];
D=+C[35];
R=+C[104]-C[103]*DP[0]+C[36]*DP[1];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[10]):Q1[10])*(gswidth_omg_p25p25_u? creal(Q1[2]):conj(
 Q1[2]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[106];
D=+C[35];
R=+C[104]-C[103]*DP[0]+C[36]*DP[1];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[10]):Q1[10])*(gswidth_omg_p25p25_u? creal(Q1[3]):conj(
 Q1[3]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[107];
D=+C[1];
R=+DP[1]*(2*(DP[1]-DP[0])-C[5])+C[109]+C[108]*DP[0];
R*=(N/D);
Prop=1*Q2[12];
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[107];
D=+C[1];
R=+DP[1]*(2*(DP[1]-DP[0])-C[5])+C[109]+C[108]*DP[0];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[12]):Q1[12])*(gtwidth_omg_p25p25_u? creal(Q1[13]):conj(
 Q1[13]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[110];
D=+C[35];
R=+2*DP[1]-C[111]-DP[0];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[12]):Q1[12])*(gswidth_omg_p25p25_u? creal(Q1[1]):conj(
 Q1[1]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[112];
D=+C[35];
R=+2*DP[1]-C[111]-DP[0];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[12]):Q1[12])*(gswidth_omg_p25p25_u? creal(Q1[2]):conj(
 Q1[2]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=-C[113];
D=+C[35];
R=+2*DP[1]-C[111]-DP[0];
R*=(N/D);
Prop=1*(gtwidth_omg_p25p25_u? creal(Q1[12]):Q1[12])*(gswidth_omg_p25p25_u? creal(Q1[3]):conj(
 Q1[3]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[114];
D=+C[115];
R=+DP[0]-C[111];
R*=(N/D);
Prop=1*Q2[1];
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[2]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[116];
D=+C[117];
R=+DP[0]-C[111];
R*=(N/D);
Prop=1*(gswidth_omg_p25p25_u? creal(Q1[1]):Q1[1])*(gswidth_omg_p25p25_u? creal(Q1[2]):conj(
 Q1[2]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[118];
D=+C[117];
R=+DP[0]-C[111];
R*=(N/D);
Prop=1*(gswidth_omg_p25p25_u? creal(Q1[1]):Q1[1])*(gswidth_omg_p25p25_u? creal(Q1[3]):conj(
 Q1[3]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[2];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[119];
D=+C[115];
R=+DP[0]-C[111];
R*=(N/D);
Prop=1*Q2[2];
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[3];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[120];
D=+C[117];
R=+DP[0]-C[111];
R*=(N/D);
Prop=1*(gswidth_omg_p25p25_u? creal(Q1[2]):Q1[2])*(gswidth_omg_p25p25_u? creal(Q1[3]):conj(
 Q1[3]));
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;
N=+C[121];
D=+C[115];
R=+DP[0]-C[111];
R*=(N/D);
Prop=1*Q2[3];
if(gswidth_omg_p25p25_u ) Prop=Prop*Q0[1]*Q0[2];
R*=creal(Prop);
 if(R>Fmax) Fmax=R; else if(R<-Fmax) Fmax=-R;
ans+=R;

}
return ans;
}

static void C37(REAL*C)
{
  REAL* V=va_omg_p25p25_u;
REAL S[38];                                                           
S[0]=V[460]*V[460];
S[1]=V[323]*V[323];
C[121]=+S[0]*S[1];
C[120]=+V[462]*V[460]*V[388]*V[323];
S[2]=V[462]*V[462];
S[3]=V[388]*V[388];
C[119]=+S[2]*S[3];
C[118]=+V[464]*V[460]*V[427]*V[323];
S[4]=V[122]*V[122];
S[5]=V[121]*V[121];
S[6]=V[119]*V[119];
S[7]=V[7]*V[7];
S[8]=V[3]*V[3];
S[9]=V[1]*V[1];
C[117]=+4*S[4]*S[5]*S[6]*S[7]*S[8]*S[9];
C[116]=+V[464]*V[462]*V[427]*V[388];
C[115]=+8*S[4]*S[5]*S[6]*S[7]*S[8]*S[9];
S[10]=V[464]*V[464];
S[11]=V[427]*V[427];
C[114]=+S[10]*S[11];
S[12]=V[466]*V[466];
C[113]=+S[12]*V[460]*V[323]*V[15];
C[112]=+S[12]*V[462]*V[388]*V[15];
S[13]=V[15]*V[15];
C[111]=+S[13];
C[110]=+S[12]*V[464]*V[427]*V[15];
S[14]=V[12]*V[12];
C[109]=+S[13]*S[14];
C[108]=+S[14];
S[15]=V[466]*V[466]*V[466]*V[466];
C[107]=+S[15];
S[16]=V[485]*V[485];
C[106]=+S[16]*V[460]*V[323];
C[105]=+S[16]*V[462]*V[388];
C[104]=+S[13]*(V[16]-2*V[15]);
C[103]=+V[16];
C[102]=+S[16]*V[464]*V[427];
C[101]=+S[13]*(V[15]*(V[16]-V[15])-S[14]);
C[100]=+V[15]*(V[16]-V[15])-S[14];
C[99]=+V[15]*(2*V[16]-4*V[15]);
C[98]=+S[16]*S[12];
S[17]=V[16]*V[16];
C[97]=+S[13]*(V[15]*(V[15]-2*V[16])+S[17]-S[14]);
C[96]=+V[15]*(V[15]-2*V[16])+S[17]+S[14];
C[95]=+S[13]*(V[15]*(3*V[15]-4*V[16])+S[17]+S[14]);
C[94]=+S[17]-S[13]-S[14];
C[93]=+V[15]*(4*V[16]-6*V[15]);
S[18]=V[485]*V[485]*V[485]*V[485];
C[92]=+S[18];
S[19]=V[506]*V[506];
C[91]=+S[19]*V[460]*V[323];
C[90]=+S[19]*V[462]*V[388];
C[89]=+S[13]*(V[17]-2*V[15]);
C[88]=+V[17];
C[87]=+S[19]*V[464]*V[427];
C[86]=+S[13]*(V[15]*(V[17]-V[15])-S[14]);
C[85]=+V[15]*(V[17]-V[15])-S[14];
C[84]=+V[15]*(2*V[17]-4*V[15]);
C[83]=+S[19]*S[12];
C[82]=+S[13]*(V[15]*(V[15]-2*V[16])+V[17]*V[16]-S[14]);
C[81]=+V[15]*(V[15]-2*V[17])+V[17]*V[16]+S[14];
C[80]=+V[15]*(2*(V[17]-V[16]-V[15]));
C[79]=+S[13]*(V[15]*(2*(-V[17]-V[16])+3*V[15])+V[17]*V[16]+S[14]);
C[78]=+V[17]*V[16]-S[13]-S[14];
C[77]=+V[15]*(2*(V[17]+V[16])-6*V[15]);
C[76]=+S[19]*S[16];
S[20]=V[17]*V[17];
C[75]=+S[13]*(V[15]*(V[15]-2*V[17])+S[20]-S[14]);
C[74]=+V[15]*(V[15]-2*V[17])+S[20]+S[14];
C[73]=+S[13]*(V[15]*(3*V[15]-4*V[17])+S[20]+S[14]);
C[72]=+S[20]-S[13]-S[14];
C[71]=+V[15]*(4*V[17]-6*V[15]);
S[21]=V[506]*V[506]*V[506]*V[506];
C[70]=+S[21];
S[22]=V[527]*V[527];
C[69]=+S[22]*V[460]*V[323];
C[68]=+S[22]*V[462]*V[388];
C[67]=+S[13]*(V[18]-2*V[15]);
C[66]=+V[18];
C[65]=+S[22]*V[464]*V[427];
C[64]=+S[13]*(V[15]*(V[18]-V[15])-S[14]);
C[63]=+V[15]*(V[18]-V[15])-S[14];
C[62]=+V[15]*(2*V[18]-4*V[15]);
C[61]=+S[22]*S[12];
C[60]=+S[13]*(V[15]*(V[15]-2*V[16])+V[18]*V[16]-S[14]);
C[59]=+V[15]*(V[15]-2*V[18])+V[18]*V[16]+S[14];
C[58]=+V[15]*(2*(V[18]-V[16]-V[15]));
C[57]=+S[13]*(V[15]*(2*(-V[18]-V[16])+3*V[15])+V[18]*V[16]+S[14]);
C[56]=+V[18]*V[16]-S[13]-S[14];
C[55]=+V[15]*(2*(V[18]+V[16])-6*V[15]);
C[54]=+S[22]*S[16];
C[53]=+S[13]*(V[15]*(V[15]-2*V[17])+V[18]*V[17]-S[14]);
C[52]=+V[15]*(V[15]-2*V[18])+V[18]*V[17]+S[14];
C[51]=+V[15]*(2*(V[18]-V[17]-V[15]));
C[50]=+S[13]*(V[15]*(2*(-V[18]-V[17])+3*V[15])+V[18]*V[17]+S[14]);
C[49]=+V[18]*V[17]-S[13]-S[14];
C[48]=+V[15]*(2*(V[18]+V[17])-6*V[15]);
C[47]=+S[22]*S[19];
S[23]=V[18]*V[18];
C[46]=+S[13]*(V[15]*(V[15]-2*V[18])+S[23]-S[14]);
C[45]=+V[15]*(V[15]-2*V[18])+S[23]+S[14];
C[44]=+S[13]*(V[15]*(3*V[15]-4*V[18])+S[23]+S[14]);
C[43]=+S[23]-S[13]-S[14];
C[42]=+V[15]*(4*V[18]-6*V[15]);
S[24]=V[527]*V[527]*V[527]*V[527];
C[41]=+S[24];
S[25]=V[548]*V[548];
C[40]=+S[25]*V[460]*V[323];
C[39]=+S[25]*V[462]*V[388];
C[38]=+S[13]*(V[19]-2*V[15]);
C[37]=+V[19];
C[36]=+2*V[15];
S[26]=V[122]*V[122]*V[122];
S[27]=V[121]*V[121]*V[121];
S[28]=V[119]*V[119]*V[119];
S[29]=V[7]*V[7]*V[7];
S[30]=V[3]*V[3]*V[3];
C[35]=+4*S[26]*S[27]*S[28]*S[29]*S[30]*V[1];
C[34]=+S[25]*V[464]*V[427];
C[33]=+S[13]*(V[15]*(V[19]-V[15])-S[14]);
C[32]=+V[15]*(V[19]-V[15])-S[14];
C[31]=+V[15]*(2*V[19]-4*V[15]);
C[30]=+S[25]*S[12];
C[29]=+S[13]*(V[15]*(V[15]-2*V[16])+V[19]*V[16]-S[14]);
C[28]=+V[15]*(V[15]-2*V[19])+V[19]*V[16]+S[14];
C[27]=+V[15]*(2*(V[19]-V[16]-V[15]));
C[26]=+S[13]*(V[15]*(2*(-V[19]-V[16])+3*V[15])+V[19]*V[16]+S[14]);
C[25]=+V[19]*V[16]-S[13]-S[14];
C[24]=+V[15]*(2*(V[19]+V[16])-6*V[15]);
C[23]=+S[25]*S[16];
C[22]=+S[13]*(V[15]*(V[15]-2*V[17])+V[19]*V[17]-S[14]);
C[21]=+V[15]*(V[15]-2*V[19])+V[19]*V[17]+S[14];
C[20]=+V[15]*(2*(V[19]-V[17]-V[15]));
C[19]=+S[13]*(V[15]*(2*(-V[19]-V[17])+3*V[15])+V[19]*V[17]+S[14]);
C[18]=+V[19]*V[17]-S[13]-S[14];
C[17]=+V[15]*(2*(V[19]+V[17])-6*V[15]);
C[16]=+S[25]*S[19];
C[15]=+S[13]*(V[15]*(V[15]-2*V[18])+V[19]*V[18]-S[14]);
C[14]=+V[15]*(V[15]-2*V[19])+V[19]*V[18]+S[14];
C[13]=+V[15]*(2*(V[19]-V[18]-V[15]));
C[12]=+S[13]*(V[15]*(2*(-V[19]-V[18])+3*V[15])+V[19]*V[18]+S[14]);
C[11]=+V[19]*V[18]-S[13]-S[14];
C[10]=+V[15]*(2*(V[19]+V[18])-6*V[15]);
S[31]=V[122]*V[122]*V[122]*V[122];
S[32]=V[121]*V[121]*V[121]*V[121];
S[33]=V[119]*V[119]*V[119]*V[119];
S[34]=V[7]*V[7]*V[7]*V[7];
S[35]=V[3]*V[3]*V[3]*V[3];
C[9]=+8*S[31]*S[32]*S[33]*S[34]*S[35];
C[8]=+S[25]*S[22];
S[36]=V[19]*V[19];
C[7]=+S[13]*(V[15]*(V[15]-2*V[19])+S[36]-S[14]);
C[6]=+V[15]*(V[15]-2*V[19])+S[36]+S[14];
C[5]=+2*S[13];
C[4]=+S[13]*(V[15]*(3*V[15]-4*V[19])+S[36]+S[14]);
C[3]=+S[36]-S[13]-S[14];
C[2]=+V[15]*(4*V[19]-6*V[15]);
C[1]=+16*S[31]*S[32]*S[33]*S[34]*S[35];
S[37]=V[548]*V[548]*V[548]*V[548];
C[0]=+S[37];
}
